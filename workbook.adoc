...
. Annotate the `GreetingAutoConfiguration` with `@EnableConfigurationProperties(GreetingProperties.class)` (This annotation ensures Spring Boot binds the values from `application.properties` into the `GreetingProperties` class. Without it, the injection will fail at runtime.)
...
. Also pass `GreetingProperties` into both `GreetingService` bean methods:
...
. ✅ Run the application . why does fail?
+
NOTE: The `LoggerGreetingService` will only be created if the `library-slf4j` module is present on the classpath.
...
. ✅ Run the application and see how the service is greeting now.
+
TIP: Try changing `workshop.greeting.type=stdout` or removing it entirely to observe how Spring Boot picks the fallback implementation (`StdOutGreetingService`).
