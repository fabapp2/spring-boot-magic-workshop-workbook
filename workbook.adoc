...
It is also possible to create custom conditions as equivalent to existing `@On...` conditions.
...
* Add a new `@Bean` method to the `GreetingAutoConfiguration` class, call it `beepGreetingService`, the return type is `GreetingService`
